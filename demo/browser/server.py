...
class SpectatorMatch:
    ...
    def _refresh_snapshot(self, state, seat_label,
        ...
        pot = bet0 + bet1
        self._set(
            hole_a=hole0 if seat_label[0] == self.label_a else hole1,
            hole_b=hole0 if seat_label[0] == self.label_b else hole1,
            board=board,
            pot=pot,
...
def main():
    ...
    p.add_argument("--opp-kind", default="openspiel",
                   choices=["openspiel", "lucy"])
    p.add_argument("--opp-model", required=True)
